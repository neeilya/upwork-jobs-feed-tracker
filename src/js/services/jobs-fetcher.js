...
export default {
  ...
  feedRequest() {
    return axios.get(
      'https://www.upwork.com/o/jobs/browse/url?payment_verified=1&exclude_terms=' +
        JSON.parse(localStorage.getItem('excludedJobs')).join('+') +
        '&initial_request=true&or_terms=' +
        JSON.parse(localStorage.getItem('selectedJobs')).join('+') +
        '&per_page=10&sort=recency',
      {
        headers: {
  ...
  fetchAndNotify() {
    this.fetch()
      .then(response => {
        const results = response.data.searchResults.jobs;
